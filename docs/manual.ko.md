...
- [05. Phase 0 → 13 — 실제로 있었던 일](#05-phase-0--13--실제로-있었던-일)
...
- **다른 컴퓨터에서 확인** (2026-08-26) — 들고 가서 열었더니 브라우저가 저절로
  떴고, 내장 예제가 아니라 **본인 시료의 실제 측정 데이터**로 소스판과 같은
  결과가 나왔습니다. 미리 설치한 것은 없습니다. 다만 그 컴퓨터에 Python이
  있었는지는 묻지 않아서, "Python이 없어도 된다"까지 주장하지는 않습니다.
- **대신 예상 못 한 것이 나왔습니다** — **백신이 잡습니다.** 무시하면 돌아가지만,
  경고를 예상하지 못한 사람은 파일을 지웁니다. 파일 하나짜리 배포판이 실행할
  때마다 54 MB를 임시 폴더에 풀고 거기서 실행하는데, 그게 악성코드와 모양이
  같아서 그렇습니다. 폴더 방식은 아무것도 풀지 않으므로 이 문제가 없고, 그래서
  **한 파일이냐 한 폴더냐는 이제 기동 시간 문제가 아니라 백신 문제**입니다
  ([7.3.1절](#731-남에게-줄-때--배포판-만들기)).
...
### Phase 11 — 같은 Jc(T)를 곡선으로

**이 단계는 도구를 실제로 써 봤더니 그래프가 틀렸기 때문에 생겼습니다.** 숫자가
틀린 게 아니라, 보이는 모양이 틀렸습니다.

Phase 10은 예측값을 측정 온도끼리 이은 선으로 그렸습니다. 써 보고 온 보고는
"피팅으로 안 보인다, 두 번째 데이터처럼 보인다"였습니다. **그 감이 맞았습니다.**
그 선은 **피팅된** λ와 **측정한** ξ를 섞어 만든 것이어서, Hc2 측정의 잔떨림이
모델이어야 할 선에 그대로 나타나고 있었습니다.

> **틀린 건 코드가 아니라 논리였습니다**
>
> Phase 10은 이렇게 판단했습니다 — 측정점 사이를 메우려면 그 자리의 ξ가
> 필요하고, ξ를 주려면 Bc2(T)의 형태를 가정해야 하는데 그건 spec 9절이 범위
> 밖이라고 한 것이다. 그러니 곡선은 못 그린다.
>
> 이건 **서로 다른 두 행동을 하나로 묶은** 것입니다.
>
> 마지막 측정점 **바깥으로** 나가는 것은 정말로 Bc2(T)를 가정하는 일입니다. 그
> 자리에는 붙잡아 주는 값이 없어서, 나오는 숫자가 곧 "내가 고른 함수 형태"
> 자체입니다. 그러나 측정점 **사이를** 메우는 것은 다릅니다. 양쪽이 측정값으로
> 묶여 있어서, 어떤 보간을 쓰든 답이 거의 달라지지 않습니다.
>
> "거의"는 주장하지 않고 **재서** 확인했습니다. 이 프로젝트의 규칙이니까요.

**무엇을 쟀고, 그 결과가 무엇을 바꿨나.** 빈 자리를 메우는 가장 당연한 방법은
ξ를 직접 보간하는 것입니다. **틀린 양입니다.** 식 (2)에서 ξ는 `Bc2^(-1/2)`이라,
Tc 근처에서 Bc2가 0으로 내려갈수록 급격히 위로 꺾입니다 — 하필 측정점이 가장
드문 구간입니다. 반면 Bc2는 T에 대해 거의 다항식이라, **Bc2를 보간한 뒤 ξ로
환산하면 같은 점으로 30~100배 정확합니다.**

| `Bc2(T)`, 점 8개 | Bc2를 보간 | ξ를 보간 |
| --- | --- | --- |
| `1 - t^2`, 흔한 형태 | 0.027 % | 2.23 % |
| `1 - t` | 0.000 %, 정확 | 2.35 % |
| `(1 - t^2)^2` | 1.52 % | 6.36 % |

어떤 보간법을 쓸지는 두 번째 측정으로 정했습니다. 3차 스플라인은 잡음이 없는
데이터에서 PCHIP보다 정확하지만, 실제 데이터처럼 1 % 산포가 들어가면 뒤집힙니다.
게다가 **모델에는 없는 진동을 세 배쯤 더 만들어 냅니다** — 측정점 사이의 출렁임은
그리는 사람만 알지 보는 사람은 진짜 물리와 구별할 수 없습니다. 그래서 PCHIP입니다.

그리고 세 번째 측정이 말해 준 것은, **이 모든 게 사실 별로 중요하지 않다**는
것입니다. ξ의 1 % 오차는 κ = 45에서 그려지는 Jc를 0.23 % 움직입니다. ξ가 식 (1)에
로그 안으로만 들어가기 때문이고, research R11이 반대 방향에서 적어 둔 바로 그
둔감함입니다. **알아 둘 가치가 있는 결론이지, 안 재도 되는 이유는 아닙니다.**

**이걸 놓쳤으면 화면이 통째로 하얘졌을 것.** 측정 범위 밖에서 곡선은 값이
없고, 물리 코어에서 그건 `NaN`입니다. 그런데 **JSON에는 NaN이 없습니다.**
파이썬 인코더는 `NaN`이라는 맨 글자를 그냥 써 버리는데, 브라우저의
`JSON.parse`는 그걸 거부합니다. 구멍 하나 때문에 그래프 하나가 아니라 **응답
전체가** 죽습니다. 그래서 API 경계에서 `null`로 바꾸고, 테스트는 **파싱하지 않은
원문**을 검사합니다 — 파싱해 버리면 그 순간 실수가 눈앞에서 사라지니까요.

같은 구분이 내려받는 파일까지 이어집니다. 그 칸은 **빈칸**으로 둡니다. `0`도
아니고 `nan`도 아닙니다. 엑셀은 빈칸을 "없음"으로, 숫자를 "측정값"으로 읽습니다.
분석이 말하기를 거부한 온도에 0이 찍혀 있으면 그건 남의 논문 그림에 들어갑니다.

> **만들었다가 도로 뺀 기능 — 이게 이 프로젝트에서 제일 중요한 장면입니다**
>
> 이 다음 질문은 자연스럽습니다. *"곡선을 0 K부터 Tc까지 끝까지 그리면 안 되나?"*
>
> **만들었습니다.** 측정 범위 밖에서는 κ를 가장자리 값에 붙잡고(Tc 근처에서
> λ와 ξ가 같은 꼴로 발산한다는 Ginzburg-Landau 극한입니다), 그 구간은 점선으로
> 그리고, 데이터의 κ 추세가 위험하다고 말하면 경고를 띄우고, CSV에 표시 열까지
> 넣었습니다.
>
> **그리고 도로 뺐습니다.** 재 본 결과가 예상 밖의 방향을 가리켰기 때문입니다.
>
> | 데이터의 κ | 외삽 오차 (고온쪽) |
> | --- | --- |
> | 거의 상수 | 1 % |
> | 온도에 따라 변함 | **190 %** |
>
> κ가 거의 상수일 때는 잘 맞습니다. **그런데 그 경우엔 이미 「κ 고정」 모드가
> 있습니다** — 대놓고 같은 가정을 하는 모드입니다. 반대로 κ가 온도에 따라 변할
> 때는 크게 틀립니다. **그게 바로 이 프로그램을 만든 이유인데 말입니다.**
>
> 즉 **필요 없을 때만 정확하고, 필요할 때는 틀립니다.** 이건 조절해서 쓸 문제가
> 아니라, 넣지 말아야 할 이유입니다.
>
> 신호는 만들면서 이미 있었습니다. 그 곡선에서 나오는 **피팅 값이 하나도 없고**
> (λ(0)·Δ(0)·Tc는 전부 측정 범위 안에서 나옵니다), 그 그림 하나를 오해받지 않게
> 하려고 **경고·임계값·CSV 열·점선 trace·안내 문장 다섯 개**를 붙이고 있었습니다.
> **안전장치가 기능보다 무거우면, 그게 답입니다.**
>
> 잰 것은 research R13에 남겼습니다. 6개월 뒤에 같은 질문이 나오면 **다시 재지
> 말고 표만 보면 됩니다.** 이게 사양 문서를 쓰는 이유입니다.

- **gate** — Jc 탭에 피팅 곡선이 그려지고, 그 곡선과 측정점별 예측값이 **두 개가
  아니라 하나의 모델**일 것. **통과** — 곡선을 만드는 그 식을 측정 온도에서
  계산하면 보고된 열이 그대로 나오는지 테스트가 확인합니다.
  191 backend + 25 e2e tests.

---

### Phase 12 — 내장 예제를 뺐습니다

**질문 하나에서 시작됐습니다.** *"이 NbTi와 Nb3Sn 데이터는 어디서 가져온 거야?"*

답은 **"아무 데서도 안 가져왔다"**였습니다. 두 파일 모두 저장소 안의 생성기가
정해진 파라미터에서 계산해 만든 것이었습니다.

숨긴 건 아니었습니다. 파일 머리글도, 생성기도, README도, 이 매뉴얼도 전부
"합성"이라고 적고 있었습니다. **그런데 그게 요점이 아니었습니다.**

> **측정 도구 옆에 놓인 데이터는 「이 도구가 무엇을 위한 것인지」로 읽힙니다**
>
> 우리 예제는 새 사용자에게 **물질 이름 두 개**를 내밀고 있었습니다.
> `nbti_like`, `nb3sn_like`. 머리글에 뭐라고 적혀 있든, **물질 이름은 그 물질을
> 측정한 것으로 읽힙니다.** 아무도 계측기를 대 본 적 없는 시료에 대한 주장을
> 하고 있었던 셈입니다.

**빼기 전에 실제 데이터를 먼저 찾아봤습니다.** 원 논문은 CC BY지만 Jc(T) 표가
없고 그림뿐이었습니다. 공개 데이터 저장소에는 s-wave 박막 데이터가 없었고,
공개된 REBCO 테이프 데이터는 **d-wave**라 spec 9절이 범위 밖으로 선언한
물질이었습니다. 남은 방법은 논문 그림에서 점을 찍어 뽑는 것뿐인데, 그러면
**새 사용자가 이 도구를 판단하는 바로 그 파일에 디지타이즈 오차가 들어갑니다.**
그래서 포기했고, 그 탐색 기록을 spec 9절에 남겨 다시 찾지 않게 했습니다.

**무엇이 사라졌나.** 예제 버튼 두 개, API 엔드포인트 두 개(`/api/examples`,
`/api/examples/{name}`), 그에 딸린 스키마와 오류 코드, exe와 도커 이미지 안의
데이터 폴더. 엔드포인트는 열 개에서 **여덟 개**가 됐습니다.

**무엇이 남았나 — 그리고 이게 모순이 아닌 이유.** 같은 합성 파일들이
`backend/tests/data/`에 그대로 있습니다. 반대하신 것은 **만들어낸 측정값을
배포하는 것**이지, 정답을 아는 데이터로 시험하는 것이 아니었습니다.

그리고 그 시험은 대체 불가능합니다. `test_whole_chain.py`는 파싱·단위 변환·
역산·피팅·진단을 **통째로** 돌리는 유일한 테스트인데, **실제 측정 데이터로는
이 검사를 할 수 없습니다.** 어떤 실제 박막도 λ(0)의 참값을 따로 아는 것이
없으므로, 실제 데이터는 "숫자가 나왔다"는 것만 확인해 줄 뿐 **"맞는 숫자가
나왔다"는 것은 확인해 주지 못합니다.**

이름도 바꿨습니다 — `weak_coupling_clean_hc2`, `strong_coupling_dirty_kappa`.
**물질 이름을 없애고 그 데이터가 만드는 「상황」으로 불렀습니다.** 같은 실수를
작게 반복하지 않기 위해서입니다.

**대가는 이렇습니다.** 처음 쓰는 사람은 이제 **파일을 준비해야만** 뭔가를 할 수
있습니다. 버튼 하나 누르면 되던 첫 1분이 나빠졌습니다. 그게 원래 FR-028이
존재했던 이유였고, 지금은 입력창의 안내문이 할 수 있는 만큼을 합니다.

> **테스트를 두 번 다시 쓴 이야기**
>
> "데이터를 주는 엔드포인트가 없다"를 검사하려고 처음엔 `/api/examples`가 404를
> 내는지 봤습니다. **통과했지만 이유가 틀렸습니다** — 단일 페이지 앱의 포괄
> 라우트가 모르는 주소에 페이지 자체를 돌려주므로 200이 나옵니다.
>
> 두 번째로 `app.routes`를 뒤졌습니다. 이번엔 `/api/parse`조차 없었습니다.
> 엔드포인트들이 하위 라우터에 들어 있어서, 목록에는 **라우터 하나**만 있었던
> 것입니다.
>
> 지금은 실제로 공개되는 OpenAPI 스키마를 읽습니다. **두 번 다 "통과했지만
> 아무것도 검사하지 않는" 테스트였고, 그건 없느니만 못합니다.**

- **gate** — 제품에서 측정값처럼 보이는 것이 아무것도 나가지 않을 것, 그리고
  생성 데이터가 필요했던 검증은 계속 돌 것. **통과** —
  191 backend + 26 e2e tests, exe와 도커 이미지에 데이터 폴더 없음,
  `test_the_api_serves_no_data_of_its_own`이 데이터 엔드포인트의 부활을 막습니다.

---

### Phase 13 — 다른 모델의 적대적 리뷰, 세 가지 지적

**다른 AI 모델(Codex)에게 "이 프로그램이 목적과 사양을 제대로 구현했는가"를
적대적으로 검토하게 했습니다.** 지적이 셋 나왔습니다.

**고치기 전에 셋 다 코드와 대조하고, 재현할 수 있는 건 재현했습니다.** 셋 모두
**본질은 맞았고, 세부는 하나씩 틀렸습니다.** 그리고 그 틀린 세부가 매번 고치는
방법을 바꿨습니다.

**① 벽에 걸린 피팅이 정상 결과처럼 보고된다 — 맞았지만 이유가 틀렸습니다.**
리뷰는 "평범한 오차 막대가 붙는다"고 했습니다. 재 보니 **247개 중 247개 모두**
결합비 오차가 50 % 이상, 중앙값 19,000 %였습니다. 피팅은 소리치고 있었는데 **말로
옮기는 곳이 없었고, 결합 영역 라벨이 그걸 무시했던** 것입니다.

그래서 기준을 결합 영역의 폭에서 끌어냈습니다 — 오차가 가장 좁은 영역의 반폭보다
크면 영역을 말하지 않습니다. 그런데 **테스트 하나가 이 규칙의 구멍을 잡았습니다.**
Tc를 고정하고 0.2 Tc까지만 잰 데이터에서 오차 막대가 작게 나오면서 결합비가 19 %
틀렸습니다. 그래서 "데이터가 Tc의 0.4배에 못 미치면"이라는 조건을 더했습니다.

> **「Tc를 고정하세요」라고 쓸 뻔했습니다**
>
> 가장 당연해 보이는 해결책입니다. 재 봤더니 **참 Tc를 줘도 결합비가 세 배
> 틀렸습니다.** 빠진 건 ρs(T) 곡선이 어떻게 휘는지이고, 그건 Tc의 1/3 위에서만
> 보입니다. 끝나는 온도를 알려 줘도 모양은 생기지 않습니다. 경고 문구는 이제
> "Tc를 고정해도 해결되지 않습니다"라고 말합니다.

> **스크린샷이 문구를 한 번 더 고쳤습니다**
>
> 처음 문구는 "Δ(0)가 정해지지 않습니다"였는데, 화면에서 바로 위 칸에
> **Δ(0) = 1.42 ± 0.12 meV — 참값 1.40, 맞는 값**이 있었습니다. 저온에서도
> 산포가 작으면 Δ(0)는 정해질 수 있고, 정해지지 않는 건 **Tc, 그래서 결합비**
> 였습니다. 재 보니 Tc가 자유일 때 Δ(0)의 자기 오차 막대는 정직했습니다. 경고
> 이름을 `COUPLING_RATIO_NOT_DETERMINED`로 바꾸고, Tc 자유/고정에 따라 Δ(0)를
> 어떻게 읽을지 따로 말하게 했습니다.

**② 내보낸 CSV로 오차 계산을 재현할 수 없다 — 맞았고, 더 있었습니다.** 리뷰가
말한 대로 입력 오차(%)가 빠져 있었습니다. 확인하다 보니 **ξ를 어떻게 정했는지와
κ 고정값도 두 파일 어디에도 없었고**, **오차 전파 쪽 경고는 아예 기록되지 않고
있었습니다.** 셋 다 넣었습니다.

**③ 늦게 온 오차 전파 결과가 다른 시료에 붙는다 — 맞았고, 화면에서는 더
나빴습니다.** 고치기 전에 재현했습니다. 계산 시작 요청을 붙잡아 둔 채 다른 시료를
분석했더니, **λ(0) = 120 nm인 피팅 옆에 250 nm짜리 오차 구간**이 붙었습니다. 이대로
CSV에 나갔을 겁니다. 방어선을 두 겹으로 넣고, **각각을 혼자 두고도 막히는지** 따로
확인했습니다.

> **테스트가 옛 코드에서 실패하는지 꼭 확인합니다**
>
> 새로 쓴 테스트가 고치기 전 코드에서 **실패하지 않으면, 그 테스트는 아무것도
> 검사하지 않는 것**입니다. 이번 세 가지 모두 옛 코드로 되돌려 실패를 확인한 뒤에
> 고쳤습니다. 2번에서는 두 번이나 "통과했지만 아무것도 검사하지 않는" 테스트를
> 먼저 썼었습니다.

**리뷰어가 할 수 없었던 것.** Codex는 자기 환경에서 Python을 띄우지 못해 **코드를
읽기만 했습니다.** 위의 메커니즘은 전부 여기서 실제로 돌려서 확인한 것입니다.
두 번째 눈은 **어디를 볼지**를 알려 주고, **무엇이 사실인지**는 재 봐야 압니다.

- **gate** — 각 지적을 고치되 고치기 전 코드에서 실패하는 테스트와 함께, 또는
  이유를 들어 기각. **통과** — 셋 다 수정. 202 backend + 28 e2e tests.
...
| 백엔드 테스트 | 202개 |
| 브라우저 테스트 | 28개 |
| API 엔드포인트 | 8개 (예제용 2개는 Phase 12에서 빠졌습니다) |
| 요구사항 | 30개 (전부 작업에 연결) |
| 내장 예제 | 없음 (Phase 12에서 뺐습니다) |
...
| | 크기 | 기동 시간 | 백신 | 받는 사람이 하는 일 |
|---|---|---|---|---|
| 파일 하나 | 54 MB | 12.6초 | **경고함** | 더블클릭 |
| 폴더 압축 | 55 MB | 5.0초 | 조용함 | 압축 풀고 안의 `.exe` 더블클릭 |

더블클릭하면 검은 창이 뜨고, 잠시 뒤 브라우저에 화면이 나옵니다. **검은 창을
닫으면 프로그램이 종료됩니다** — 받는 사람에게 있는 유일한 정지 버튼이라
일부러 남겨 두었습니다.

> **백신이 경고합니다. 받는 사람에게 미리 말해 주세요**
>
> 실제로 다른 컴퓨터에 들고 갔더니 백신이 잡았습니다. **무시하고 실행하면
> 정상적으로 돌아갑니다.** 문제는 그걸 모르는 사람은 파일을 지우고 "이거 안
> 되는데요"라고 한다는 점입니다.
>
> 백신이 틀린 것도 아닙니다. 파일 하나짜리 배포판은 실행할 때마다 자기 안의
> 54 MB를 임시 폴더에 풀고 거기서 실행합니다. **악성코드가 하는 짓과 모양이
> 똑같습니다.** 파일만 봐서는 구별할 방법이 없습니다. UPX 압축을 애초에 꺼 둔
> 것도 그것 역시 같은 종류의 신호이기 때문입니다.
>
> **폴더 방식은 시작할 때 아무것도 풀지 않아서 이 문제가 없습니다.** 이제
> 폴더 방식을 택할 이유가 하나 더 생겼고, 사실 이쪽이 12.6초 대 5.0초보다
> 더 실질적인 이유입니다. **남에게 줄 때는 압축 파일로 주세요.**
>
> 경고를 완전히 없애려면 돈을 주고 인증서를 사서 exe에 서명해야 합니다. 그건
> 하지 않았으므로, 이건 "해결된 문제"가 아니라 "설명해야 하는 문제"입니다.
...
**시작 화면.** 데이터를 붙여넣거나 파일을 엽니다. **누를 예제는 없습니다** —
Phase 12에서 뺐습니다. 대신 입력창이 기대하는 열 순서와 구분자를 직접 보여
줍니다. 아래 설정 패널에서 Jc 단위,
...
![데이터를 붙여넣은 화면](images/02-example-loaded.png)

**데이터를 붙여넣은 직후.** 표를 읽자마자 열 미리보기가 나타나, 물리 계산이 시작되기
...
**임계전류밀도 Jc(T).** 넣은 값 그 자체와, 그 위를 지나가는 피팅 곡선입니다.
앞의 두 그래프는 *유도된* 양을 보여 주는데 이것만 **측정한 값 자체**와의
비교입니다. 값들은 결과 CSV의 `Jc_A_per_m2`와 `Jc_model_A_per_m2` 두 열에 나란히
들어 있고, 곡선 자체는 곡선 CSV의 `Jc_model_A_per_m2` 열에 있습니다.

**곡선이 측정 범위에서 끊깁니다.** 위 그림에서 곡선은 가장 낮은 측정 온도에서
시작해 가장 높은 측정 온도에서 끝납니다. ρs와 λ 곡선이 0 K부터 Tc까지 가는 것과
다릅니다. 식 (1)에는 λ 외에 ξ가 필요한데, ξ가 피팅에서 나오는 것은 「κ 고정」
모드뿐이기 때문입니다.

| ξ를 정하는 방식 | 곡선이 그려지는 범위 |
| --- | --- |
| 고정 κ | **0 K ~ Tc 전체** — ξ = λ_model(T)/κ, 피팅 결과만으로 나옴 |
| Hc2 입력 | 측정 온도 범위 안에서만 |
| ξ 직접 입력 | 측정 온도 범위 안에서만 |

![κ를 고정했을 때의 임계전류밀도 그래프](images/06b-chart-jc-fixed-kappa.png)

κ를 고정한 두 번째 시험 데이터입니다. 같은 탭인데 곡선이 0 K에서
시작해서 오른쪽 아래로 화면 밖까지 내려갑니다. **κ를 고정하면 곡선을 어디서나
그릴 수 있는 이유는 필요한 숫자를 가정해 버렸기 때문**이고, 이 그림이 그 차이를
눈으로 보여 주는 유일한 자리입니다.

> **측정점 사이를 메우는 것과, 측정 범위 밖으로 나가는 것은 다릅니다**
>
> 처음에는 둘 다 거절했습니다. 잘못된 판단이었습니다.
>
> 측정점 **사이**는 양쪽이 측정값으로 묶여 있어서, 어떤 보간을 쓰든 결과가 거의
> 같습니다 — 재 보니 그려지는 Jc 기준으로 0.35 % 안쪽입니다. 측정 범위 **밖**은
> 붙잡아 주는 값이 한쪽에도 없어서, 나오는 값이 곧 "내가 고른 함수 형태"
> 그 자체입니다. 그건 spec 9절이 범위 밖이라고 못박은 가정입니다.
>
> 그래서 지금은 **사이는 메우고, 밖으로는 나가지 않습니다.**
...
적용되지 않는다는 경고입니다(헌법 VI조). 두 번째는 산포가 큰 쪽 데이터의 결과로,
...
```powershell
# 백엔드 테스트 202개 — 물리가 맞는지
cd backend
.\.venv\Scripts\python.exe -m pytest -q

# 브라우저 테스트 28개 — 서버 둘을 알아서 켜고 실제 화면을 조작합니다
cd frontend
npm run test:e2e
...
- **Docker 이미지는 빌드된 적이 없습니다.** Docker Desktop 미설치. `tasks.md`의
  Phase 7 gate에 무엇을 대신 확인했는지 정확히 적혀 있습니다. 이제 이 목록에
  남은 **유일한** 항목입니다.
- **배포판 exe를 들고 간 컴퓨터에 Python이 있었는지는 묻지 않았습니다.** 아래
  「닫힌 항목」의 첫 번째와 이어지는 이야기입니다. 실행 자체는 확인됐지만,
  "Python이 없어도 된다"는 주장 그 자체는 이 관찰만으로 증명되지 않습니다.

#### 닫힌 항목 — 어떻게 닫혔는지

- **배포판 `.exe`가 다른 컴퓨터에서 돕니다.** (2026-08-26) 들고 가서 열었더니
  브라우저가 저절로 떴고, 내장 예제가 아니라 **본인이 측정한 실제 데이터**를
  넣어서 소스판과 같은 결과를 얻었습니다. 그 컴퓨터에 미리 설치한 것은
  없습니다.

  대신 **예상하지 못한 것이 하나 나왔습니다: 백신이 잡습니다.** 무시하면
  돌아가지만, 모르는 사람은 파일을 지웁니다. 자세한 내용과 대처는
  [7.3.1절](#731-남에게-줄-때--배포판-만들기)에 적었습니다. 이 발견이 실행
  확인 자체보다 실용적으로 더 중요합니다.

- **Git 원격 저장소가 생겼습니다.** (2026-08-26)
  <https://github.com/KTKim1112/nodeless-sc-gap> — 공개, MIT. OneDrive를
  떠나면서 잃었던 백업이 이걸로 대체되었고, 이제 `git push` 한 줄이면 됩니다.
  이 컴퓨터가 고장 나도 저장소는 남습니다.
...
# ──────── 검증 ────────
cd backend; .\.venv\Scripts\python.exe -m pytest -q     # 202개
cd frontend; npm run test:e2e                           # 28개
