...
- [05. Phase 0 → 7 — 실제로 있었던 일](#05-phase-0--7--실제로-있었던-일)
...
| **제작** | Phase 1부터 7까지, 각 단계마다 통과 조건(gate)을 걸고 순서대로 | 테스트 189개가 통과하는, 브라우저에서 도는 프로그램 |
...
│   │   ├── api/routes.py     엔드포인트 9개
│   │   └── schemas.py        입출력 형식. 단위 변환은 여기서만
│   ├── tests/                테스트 13개 파일, 166개
...
```powershell
# 백엔드 테스트 166개 — 물리가 맞는지
cd backend
.\.venv\Scripts\python.exe -m pytest -q

# 브라우저 테스트 23개 — 서버 둘을 알아서 켜고 실제 화면을 조작합니다
cd frontend
npm run test:e2e
...
# ──────── 검증 ────────
cd backend; .\.venv\Scripts\python.exe -m pytest -q     # 166개
cd frontend; npm run test:e2e                           # 23개
